...
function BopComments_getRightMList()
{
    global $xoopsDB, $xoopsUser;

    $modulepermHandler = xoops_getHandler('groupperm');

    $sql = 'SELECT mid, dirname, name FROM ' .
...
function BopComments_arrayMarge($array)
{
    $topics = [];

    foreach ($array as $value) {
        foreach ($value as $key => $topicvalue) {
            $setkey = BopComments_KeyCheck($topics, $key);
...
function BopComments_minimum($array, $maxtopic)
{
    krsort($array);
...
function BopComments_finalize($array, $maxtopic, $userinfosetting, $catmax, $topicmax, $tsformat)
{
    krsort($array);
    ...
    foreach ($array as $key => $value) {
        ...
        $newarray[$key]['userprof'] = BopComments_getUserprof($newarray[$key]['user']);

        $newarray[$key]['user'] = BopComments_getUserInfo($newarray[$key]['user'], $userinfosetting);

        $newarray[$key]['place'] = BopComments_strmini($newarray[$key]['place'], $catmax);

        $newarray[$key]['topic'] = BopComments_strmini($newarray[$key]['topic'], $topicmax);
...
function BopComments_KeyCheck($array, $key)
{
...
function BopComments_getUserInfo($userid, $option)
{
    if (1 == $option) {
        if (0 == !$userid) {
...
function BopComments_getUserprof($userid)
{
    if (0 == !$userid) {
...
function BopComments_strmini($string, $maxlong)
{
    if (0 == !$maxlong && BopComments_jstrlen($string) > $maxlong) {
        return BopComments_jsubstr($string, 0, $maxlong) . '...';
    }
...
*/
function BopComments_jsubstr($str, $start = 0, $length = 0)
{
    $b = unpack('C*', $str);
...
/*
    ...
    for using shift_jis encoding, remove comment string.
*/
function BopComments_jstrlen($str)
{
    $b = unpack('C*', $str);
