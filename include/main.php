...
function BopComments_main($option)
{
    $plugin_dir = XOOPS_ROOT_PATH . '/modules/BopComments/plugin/';

    $pluginimg_dir = XOOPS_ROOT_PATH . '/modules/BopComments/plugin/image/';

    $pluginimg_http = XOOPS_URL . '/modules/BopComments/plugin/image/';

    //使用権限のあるモジュールリストを取得する

    $mod_lists = BopComments_getRightMList();
    ...
    foreach ($mod_lists as $mod => $module) {
        ...
        if (file_exists($mod_plugin_file)) {
            ...
            if (function_exists('b_BopComments_' . $mod)) {
                //コメント情報を各モジュールごとに表示件数の数だけ得る。

                $comment[] = call_user_func('b_BopComments_' . $mod, $option, $module['moduleid']);
