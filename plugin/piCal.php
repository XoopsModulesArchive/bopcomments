...
/**
 * BopComments v0.5x,0.6x plugin for piCal (0.5x/0.4x
 *
 ...
 * @copyright copyright (c) 2003 project-bop.
 */

// このプラグインは 終了したイベントについては表示しません。
function b_BopComments_piCal($maxtopic)
{
    ...
    $SQL = 'SELECT id, uid, summary, UNIX_TIMESTAMP(dtstamp) as dtstamp FROM ' .
            $xoopsDB->prefix('pical_event') .
            ' WHERE end NOT BETWEEN 0 and ' . time() . ' AND (rrule_pid=0 OR rrule_pid=id) ' .
            " AND admission<>0 AND $whr_class " . // GIJ added line
    ...
    while ($result = $xoopsDB->fetchArray($query)) {
        $key = BopComments_KeyCheck($comment, $result['dtstamp']);

        $id = (int) $result['id'];

        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/piCal.gif';
