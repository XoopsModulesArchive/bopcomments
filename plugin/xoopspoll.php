...
function b_BopComments_xoopspoll($maxtopic, $module_mid)
{
    ...
    $SQL = 'SELECT count(*) AS reply, max(com.com_id) as comid, p.poll_id, p.question, p.user_id, p.votes, max(com.com_created) AS time FROM ' .
            $xoopsDB->prefix('xoopspoll_desc') . ' p LEFT JOIN ' .
            $xoopsDB->prefix('xoopscomments') . ' com ON p.poll_id=com.com_itemid ' .
            'WHERE com.com_status=2 AND com.com_modid=' . $module_mid . ' ' .
    ...
    while (false !== ($result = $xoopsDB->fetchArray($query))) {
        ...
        $p_SQL = 'SELECT max(time) as polltime FROM ' .
                $xoopsDB->prefix('xoopspoll_log') .
                ' WHERE poll_id=' . $result['poll_id'];
        ...
        $key = BopComments_KeyCheck($comment, max($result['time'], $log_result['polltime']));

        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/xoopspoll.gif';
    ...
    $SQL = 'SELECT poll_id, question, user_id, votes, start_time FROM ' .
            $xoopsDB->prefix('xoopspoll_desc') .
            ' WHERE poll_id NOT IN (' . implode(',', $oncom) . ') ' .
    ...
    while (false !== ($result = $xoopsDB->fetchArray($query))) {
        $p_SQL = 'SELECT max(time) as time FROM ' .
                $xoopsDB->prefix('xoopspoll_log') .
                ' WHERE poll_id=' . $result['poll_id'];
        ...
        $key = BopComments_KeyCheck($comment, max($result['start_time'], $log_result['time']));
