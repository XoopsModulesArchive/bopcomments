...
function b_BopComments_myalbum($maxtopic, $module_mid)
{
    ...
    $c_SQL = 'SELECT count(*) as reply, max(com.com_id) as comid, p.lid, p.title as ptitle, p.hits, cat.cid as catid, cat.title as cattitle, max(com.com_created) as time FROM (' .
        $xoopsDB->prefix('myalbum_photos') . ' p LEFT JOIN ' .
        $xoopsDB->prefix('myalbum_cat') . ' cat ON p.cid=cat.cid) LEFT JOIN ' .
        $xoopsDB->prefix('xoopscomments') . ' com ON com.com_itemid=p.lid ' .
        'WHERE p.status=1 AND com.com_status=2 AND com.com_modid=' . $module_mid . ' ' .
    ...
    while ($result = $xoopsDB->fetchArray($query2)) {
        ...
        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/myalbum.gif';
    ...
    $SQL = 'SELECT p.lid, p.submitter, p.title as ptitle, p.date, p.hits, cat.cid as catid, cat.title as cattitle FROM ' .
            $xoopsDB->prefix('myalbum_photos') . ' p LEFT JOIN ' .
            $xoopsDB->prefix('myalbum_cat') . ' cat ON p.cid=cat.cid ' .
            'WHERE p.status=1 AND p.lid NOT IN (' . implode(',', $oncom) . ') ' .
