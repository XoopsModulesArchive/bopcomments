...
function b_BopComments_mylinks($maxtopic, $module_mid)
{
    ...
    $c_SQL = 'SELECT count(*) as reply, max(com.com_id) AS comid, l.lid,l.cid,l.date,l.title as ltitle, l.submitter, l.date,l.hits,l.votes,c.title as ctitle, max(com.com_created) as time FROM (' .
            $xoopsDB->prefix('mylinks_links') . ' l  LEFT JOIN ' .
            $xoopsDB->prefix('mylinks_cat') . ' c ON l.cid=c.cid) LEFT JOIN ' .
            $xoopsDB->prefix('xoopscomments') . ' com ON com.com_itemid=l.lid ' .
            'WHERE com.com_status=2 AND com.com_modid=' . $module_mid . ' AND l.status>0 AND l.comments>0 ' .
    ...
    while ($result = $xoopsDB->fetchArray($query2)) {
        ...
        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/mylinks.gif';
    ...
    $SQL = 'SELECT l.lid,l.cid,l.title as ltitle, l.submitter, l.date,l.hits,l.votes,c.title as ctitle FROM ' .
            $xoopsDB->prefix('mylinks_links') . ' l LEFT JOIN ' .
            $xoopsDB->prefix('mylinks_cat') . ' c ON l.cid=c.cid ' .
            'WHERE l.lid NOT IN (' . implode(',', $oncom) . ') AND l.status>0 AND l.comments=0 ' .
