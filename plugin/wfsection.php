...
// $Id: wfsection.php,v 0.6.1 2003/10/25 01:10:00 BoBpp Exp $
function b_BopComments_wfsection($maxtopic, $module_mid)
{
    ...
    $c_SQL = 'SELECT count(*) as reply, max(com.com_id) as comid, a.articleid, a.uid, a.title as atitle, a.counter, cat.id as catid, cat.title as cattitle, max(com.com_created) as time FROM (' .
        $xoopsDB->prefix('wfs_article') . ' a LEFT JOIN ' .
        $xoopsDB->prefix('wfs_category') . ' cat ON a.categoryid=cat.id) LEFT JOIN ' .
        $xoopsDB->prefix('xoopscomments') . ' com ON com.com_itemid=a.articleid ' .
        "WHERE a.offline=0 AND a.groupid REGEXP '[" . implode('', $groupid) . "]' AND com.com_status=2 AND com.com_modid=" . $module_mid . ' AND a.expired NOT BETWEEN 1 AND ' . time() . ' AND a.published<' . time() . ' ' .
    ...
    while ($result = $xoopsDB->fetchArray($query2)) {
        ...
        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/wfsection.gif';
    ...
    $SQL = 'SELECT a.articleid, a.uid, a.title as atitle, a.created, a.counter, cat.id as catid, cat.title as cattitle FROM ' .
            $xoopsDB->prefix('wfs_article') . ' a LEFT JOIN ' .
            $xoopsDB->prefix('wfs_category') . ' cat ON a.categoryid=cat.id ' .
            "WHERE a.offline=0 AND a.groupid REGEXP '[" . implode('', $groupid) . "]' AND a.articleid NOT IN (" . implode(',', $oncom) . ') AND a.expired NOT BETWEEN 1 AND ' . time() . ' AND a.published<' . time() . ' ' .
