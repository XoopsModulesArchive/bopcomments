...
function b_BopComments_xoopsfaq($maxtopic, $module_mid)
{
    ...
    $SQL = 'SELECT count(*) AS reply, max(com.com_id) AS comid, c.contents_id, c.category_id AS catid, c.contents_title, cat.category_id, cat.category_title, max(com.com_created) AS time FROM (' .
            $xoopsDB->prefix('xoopsfaq_contents') . ' c LEFT JOIN ' .
            $xoopsDB->prefix('xoopsfaq_categories') . ' cat ON c.category_id=cat.category_id) LEFT JOIN ' .
            $xoopsDB->prefix('xoopscomments') . ' com ON c.contents_id=com.com_itemid ' .
            'WHERE com.com_status=2 AND com.com_modid=' . $module_mid . ' AND c.contents_visible=1 ' .
    ...
    while (false !== ($result = $xoopsDB->fetchArray($query))) {
        ...
        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/xoopsfaq.gif';
    ...
    $SQL = 'SELECT c.contents_id, c.category_id AS catid, c.contents_title, cat.category_id, c.contents_time, cat.category_title FROM ' .
            $xoopsDB->prefix('xoopsfaq_contents') . ' c LEFT JOIN ' .
            $xoopsDB->prefix('xoopsfaq_categories') . ' cat ON c.category_id=cat.category_id ' .
            'WHERE c.contents_id NOT IN (' . implode(',', $oncom) . ') AND c.contents_visible=1 ' .
    ...
    while (false !== ($result = $xoopsDB->fetchArray($query))) {
        $key = BopComments_KeyCheck($comment, $result['contents_time']);
