...
function b_BopComments_system($maxtopic)
{
    $xoopsDB = XoopsDatabaseFactory::getDatabaseConnection();

    $comment = [];

    $SQL = 'SELECT uid, uname, user_regdate, posts FROM ' .
    ...
    while (false !== ($userData = $xoopsDB->fetchArray($userQ))) {
        $key = BopComments_KeyCheck($comment, $userData['user_regdate']);

        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/system.gif';
