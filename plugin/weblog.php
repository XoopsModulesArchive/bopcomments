...
function b_BopComments_weblog($maxtopic, $module_mid)
{
    ...
    $c_SQL = 'SELECT count(*) as reply, max(com.com_id) as comid, w.blog_id, w.title, w.reads, cat.cat_id, cat.cat_title, max(com.com_created) as time FROM (' .
        $xoopsDB->prefix('weblog') . ' w LEFT JOIN ' .
        $xoopsDB->prefix('weblog_category') . ' cat ON w.cat_id=cat.cat_id) LEFT JOIN ' .
        $xoopsDB->prefix('xoopscomments') . ' com ON com.com_itemid=w.blog_id ' .
        'WHERE w.private=0 AND com.com_status=2 AND com.com_modid=' . $module_mid . ' ' .
    ...
    while ($result = $xoopsDB->fetchArray($query2)) {
        ...
        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/weblog.gif';
    ...
    $SQL = 'SELECT w.blog_id, w.user_id, w.title, w.created, w.reads, cat.cat_id, cat.cat_title FROM ' .
            $xoopsDB->prefix('weblog') . ' w LEFT JOIN ' .
            $xoopsDB->prefix('weblog_category') . ' cat ON w.cat_id=cat.cat_id ' .
            'WHERE w.private=0 AND w.blog_id NOT IN (' . implode(',', $oncom) . ') ' .
