...
function b_BopComments_mydownloads($maxtopic, $module_mid)
{
    ...
    $c_SQL = 'SELECT count(*) as reply, max(com.com_id) AS comid, d.lid, d.cid, d.date, d.title as dtitle, d.submitter, d.date,d.hits,d.votes,c.title as ctitle, max(com.com_created) as time FROM (' .
            $xoopsDB->prefix('mydownloads_downloads') . ' d  LEFT JOIN ' .
            $xoopsDB->prefix('mydownloads_cat') . ' c ON d.cid=c.cid) LEFT JOIN ' .
            $xoopsDB->prefix('xoopscomments') . ' com ON com.com_itemid=d.lid ' .
            'WHERE com.com_status=2 AND com.com_modid=' . $module_mid . ' AND d.status>0 AND d.comments>0 ' .
    ...
    while ($result = $xoopsDB->fetchArray($query2)) {
        ...
        $key = BopComments_KeyCheck($comment, max($result['date'], $result['time']));

        $comment[$key]['moduleimg'] = XOOPS_URL . '/modules/BopComments/plugin/image/mydownloads.gif';
    ...
    $SQL = 'SELECT d.lid, d.cid, d.title as dtitle, d.submitter, d.date, d.hits, d.votes, c.title as ctitle FROM ' .
            $xoopsDB->prefix('mydownloads_downloads') . ' d LEFT JOIN ' .
            $xoopsDB->prefix('mydownloads_cat') . ' c ON d.cid=c.cid ' .
            'WHERE d.lid NOT IN (' . implode(',', $oncom) . ') AND d.status>0 AND d.comments=0 ' .
    ...
    while ($result = $xoopsDB->fetchArray($query)) {
        $key = BopComments_KeyCheck($comment, $result['date']);
